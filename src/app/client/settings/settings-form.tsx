...
import { updateClientAction } from '@/app/agency/clients/[id]/actions'
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await updateClientAction(client.id, {
        ...
        meta_form_id: client.meta_form_id ?? '',
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
      ...
      <Section title="Dados da clínica">
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Telefone" name="contact_phone" defaultValue={client.contact_phone ?? ''} />
          <Field label="Email" name="contact_email" type="email" defaultValue={client.contact_email ?? ''} />
        </div>
      </Section>

      <Section title="WhatsApp">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Instância" name="whatsapp_instance" defaultValue={client.whatsapp_instance ?? ''} placeholder="nome-da-instancia" />
          <Field label="Número" name="whatsapp_phone" defaultValue={client.whatsapp_phone ?? ''} placeholder="5511999999999" />
        </div>
      </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-3">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4">{children}</div>
    </div>
  )
}
